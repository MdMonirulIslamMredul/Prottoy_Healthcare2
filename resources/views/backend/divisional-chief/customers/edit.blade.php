...
<div class="container-fluid">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('divisionalchief.customers.update', $customer->id) }}" method="POST">
                @csrf
                @method('PUT')
                            ...
                            <select class="form-select @error('pho_id') is-invalid @enderror"
                                ...
                                @foreach($phos as $pho)
                                    <option value="{{ $pho->id }}"
                                        ...
                                        {{ $pho->name }} - {{ $pho->district->name ?? 'N/A' }} - {{ $pho->upzila->name ?? 'N/A' }}
                                    </option>
                                @endforeach
                            </select>
                            @error('pho_id')
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control"
                                   value="{{ $customer->division->name ?? 'N/A' }}"
                                   readonly>
                        </div>
                    </div>
