...
<div class="container-fluid">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('districtmanager.upazila-supervisors.store') }}" method="POST">
                @csrf
                ...
                <div class="row">
                            ...
                            <select class="form-select @error('upzila_id') is-invalid @enderror"
                                ...
                                @foreach($upzilas as $upzila)
                                    <option value="{{ $upzila->id }}"
                                            {{ old('upzila_id') == $upzila->id ? 'selected' : '' }}
                                            {{ in_array($upzila->id, $assignedUpzilas) ? 'disabled' : '' }}>
                                        {{ $upzila->name }} {{ in_array($upzila->id, $assignedUpzilas) ? '(Already Assigned)' : '' }}
                                    </option>
                                @endforeach
                            </select>
                            @error('upzila_id')
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            <label class="form-label">District (Inherited)</label>
