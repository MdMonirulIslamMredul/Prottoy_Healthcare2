...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>Add New PHO</h2>
        <a href="{{ route('districtmanager.phos.index') }}" class="btn btn-secondary">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('districtmanager.phos.store') }}" method="POST">
                @csrf
                ...
                <div class="row">
                            ...
                            <select class="form-select @error('upazila_supervisor_id') is-invalid @enderror"
                                ...
                                @foreach($upazilaSupervisors as $supervisor)
                                    <option value="{{ $supervisor->id }}"
                                        ...
                                        {{ $supervisor->name }} - {{ $supervisor->upzila->name ?? 'N/A' }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            <label class="form-label">District/Division (Inherited)</label>
                            <input type="text" class="form-control"
                                   value="{{ $districtManager->district->name ?? 'N/A' }}, {{ $districtManager->division->name ?? 'N/A' }}"
                                   readonly>
                        </div>
                    </div>
                ...
                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('districtmanager.phos.index') }}" class="btn btn-secondary">Cancel</a>
